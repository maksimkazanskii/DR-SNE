...
OUT_DIR = "output/ablation_k/3run"
...
def load_pbmc(seed):
    ...
    sc.tl.pca(adata, n_comps=PCA_FIXED, random_state=seed)
...
def run_single_ablation(seed):
    ...
    for k in K_LIST:
        ...
        # 🔥 Match perplexity to k (important!)
        P = compute_P(X, perplexity=k)
        ...
        cont = continuity(X, Z, n_neighbors=k)
...
def aggregate_results(all_runs):
    ...
    for run in all_runs:
        for row in run:
            ...
            if key not in agg:
                agg[key] = {k: [] for k in row if k != "k"}

            for k_metric, v in row.items():
                if k_metric != "k":
                    agg[key][k_metric].append(v)
    ...
    for key, metrics in agg.items():
        ...
        for k_metric, values in metrics.items():
            row[f"{k_metric}_mean"] = np.nanmean(values)
            row[f"{k_metric}_std"] = np.nanstd(values)

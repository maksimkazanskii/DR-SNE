...
OUT_DIR = "output/ablation_pareto_seeds"
...
def load_mnist(seed, n_samples=5000):
    np.random.seed(seed)
...
def load_fashion_mnist(seed, n_samples=5000):
    np.random.seed(seed)
...
def load_synthetic_density(seed, n_samples=5000, n_clusters=4, dim=50):
    np.random.seed(seed)
...
def load_spiral_density(seed, n_samples=5000):
    np.random.seed(seed)
...
# CORE
# =========================================================
def run_pareto_ablation(X, lambdas, seed):
    np.random.seed(seed)
...
def save_results(dataset_name, seed, results):
    path = os.path.join(OUT_DIR, f"{dataset_name}_seed{seed}.csv")

    with open(path, "w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["seed", "lambda", "trustworthiness", "density_corr"]
        )
        writer.writeheader()
...
def main():
    ...
    for name, loader in datasets:
        ...
        for seed in SEEDS:
            ...
            save_results(name, seed, results)
